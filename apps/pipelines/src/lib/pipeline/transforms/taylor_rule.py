...
def build_taylor_rule_inputs(staging_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for region, (policy_series_key, inflation_series_key) in sorted(region_series_map.items()):
        ...
        inflation_row = _latest_row_by_series(staging_rows, inflation_series_key)

        policy_rate = float(policy_row["numeric_value"])
        inflation = float(inflation_row["numeric_value"])
        implied_rate = round(
        ...
        mart_rows.append(
            {
                "region": region,
                "as_of_date": max(str(policy_row["observation_date"]), str(inflation_row["observation_date"])),
